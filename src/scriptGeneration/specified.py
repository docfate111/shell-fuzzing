...
# print(NRef("s"))
NDef("~", "~")
NDef("tilde", WeightedOr((NRef("s"), 0.8), (NRef("parens"), 0.1), (NRef("~"), 0.1)))
# (And(
# "~",
...
# never used?
NDef(
    "quotedcharswithsingleordoublequotes",
    Or(
...
NDef(
    ...
    And(
        ...
        Or(
            And("$", NRef("globalvar")),
            And(
                String(charset=String.charset_alpha + b"_", min=1, max=1),
...
NDef(
    "NAME",
    String(charset=String.charset_alpha + String.charset_alphanum, min=1, max=20),
)
...
NDef(
    "parens",
    Or(And("$ ", NRef("possible_commands"), " ", NRef("recursableparens"))),
    And("$( ", NRef("possible_commands"), " ) "),
    And("$(( ", NRef("possible_commands"), " )) "),
)
# Rules for tilde:
NDef("~orVariable", Or(NRef("tilde"), NRef("parens")))
"""
/* -------------------------------------------------------
   The grammar symbols
   ------------------------------------------------------- */
...
NDef(
    "ASSIGNMENT_WORD",
    And(
        Or(NRef("NAME"), And("$", NRef("globalvar")), NRef("WORD")),
        " = ",
        Or(
...
NDef(
    "IO_NUMBER",
    And(UInt(odds=[(0.45, [0, 2]), (0.45, [3, 9]), (0.1, [10, 65535])]), " "),
)
"""
...
Def(
    "program",
    WeightedOr(
        (And(NRef("linebreak"), NRef("complete_commands"), NRef("linebreak")), 0.98),
        (NRef("linebreak"), 0.02),
    ),
    cat="start",
)
...
NDef(
    ...
    WeightedOr(
        (NRef("list"), 0.2),
        (And(NRef("list"), " ", NRef("separator_op"), " ", NRef("list")), 0.4),
        (And(NRef("command"), NRef("separator_op")), 0.4),
    ),
)
"""
...
NDef(
    "list",
    Or(
        And(NRef("list"), NRef("separator_op"), NRef("and_or")),
        NRef("and_or"),
        NRef("command"),
    ),
)
...
pipe_sequence    :                             command
                 ...
                 ;
"""
NDef("pipeline", Or(NRef("pipeline"), And(NRef("Bang"), " ", NRef("pipe_sequence"))))
NDef(
    "pipe_sequence",
    Or(
        NRef("command"),
...
NDef("wordlist", NRef("possible_commands"))  # WeightedOr(
# ,
# (And(
# causes error:
# NRef("wordlist"),
...
NDef(
    "case_list_ns",
    Or(And(NRef("case_list"), " ", NRef("case_item_ns")), NRef("case_item_ns")),
)

# avoid left recursion
NDef("case_list", Or(And(NRef("case_list"), NRef("case_item")), Or(NRef("case_item"))))
...
NDef(
    "function_body",
    Or(NRef("compound_command"), And(NRef("compound_command"), NRef("redirect_list"))),
)
...
NDef(
    "cmd_prefix",
    Or(NRef("io_redirect"), NRef("ASSIGNMENT_WORD")),
)
...
NDef(
    "redirect_list",
    Or(NRef("io_redirect"), And(NRef("redirect_list"), NRef("io_redirect"))),
)
NDef(
    "io_redirect",
    Or(
...
NDef(
    ...
    Or(
        And(
            NRef("DLESS"), NRef("here_end")
        ),  # TODO needs more! generate the contents of the heredoc (a bunch of words on lines) followed by the delimiter. delimiter may be quoted
...
NDef(
    "separator", Or(And(NRef("separator_op"), NRef("linebreak")), NRef("newline_list"))
)
